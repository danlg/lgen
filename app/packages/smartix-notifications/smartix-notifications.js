...
Smartix.Notifications.Helpers.sumOfNewChatMessageCounter = function(){
    ...
    let newMessageCount =  Notifications.find({
        'eventType':'newchatmessage',
        'hasRead': false,
        'groupId':{ $in: groupIdsInNamespace }}).count();
...
Smartix.Notifications.Helpers.sumOfNewAttendanceCounter = function(){
    let schoolId = UI._globalHelpers['getCurrentSchoolId']();
    let newMessageCount =  Notifications.find({
        'eventType':'attendance','hasRead':false,
        'namespace':schoolId
    }).count();
    if(newMessageCount > 0 ){
        log.info("sumOfNewAttendanceCounter", schoolId,", count=", newMessageCount);
        return newMessageCount;
    }else{
        return false;
    }
...
Smartix.Notifications.Helpers.sumOfNewAttendanceApprovedCounter = function(){
    let schoolId = UI._globalHelpers['getCurrentSchoolId']();
    let newMessageCount =  Notifications.find({
        'eventType':'attendance','eventSubType':'attendanceApproved',
        'hasRead':false,
        'namespace': schoolId
    }).count();
    if(newMessageCount > 0 ){
