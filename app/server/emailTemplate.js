...
newClassMailTemplate = function (to, classname, classCode) {
  ...
  log.info("Sending newClassMailTemplate:" + classCode + " to " + to);
...
testMail = function (to, title, classname) {
  Email.send({
    "from": Meteor.settings.FROM_EMAIL,
    "to": to,
    "subject": title,
...
resetPasswordEmailTemplate = function (userObj, resetPwdEmailURL) {
  var emailLang = userObj.lang || "en";
  return Spacebars.toHTML(
    {
